...
export default function DashboardLayout({ children }) {
  ...
  const navItem = (label, onClick) => (
    <p onClick={onClick} style={{ cursor: "pointer", padding: "10px 14px", borderRadius: 6, fontSize: 15, color: "white", margin: 0, marginBottom: 4 }}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", fontFamily: "sans-serif" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", backgroundColor: "#111", color: "white", position: "sticky", top: 0, zIndex: 100 }}>
        <button onClick={() => setMenuOpen(!menuOpen)} style={{ background: "none", border: "none", color: "white", fontSize: 24, cursor: "pointer", padding: "0 8px", lineHeight: 1 }}>
          {menuOpen ? "✕" : "☰"}
        </button>
        <span style={{ fontWeight: "bold", fontSize: 16 }}>Entre PRO Market</span>
        <button onClick={logout} style={{ padding: "6px 12px", backgroundColor: "#701890", color: "white", border: "none", borderRadius: 6, cursor: "pointer", fontSize: 13, fontWeight: "bold" }}>
          Log Out
        </button>
      </div>
      ...
      {/* SIDEBAR — bottom Log Out raised above device nav bars with safe-area padding */}
      <div style={{
        ...
        <div style={{ marginTop: "auto", paddingBottom: 16 }}>
          <p onClick={logout}
            style={{ cursor: "pointer", color: "#ff6b6b", padding: "12px 14px", borderRadius: 6, fontSize: 15, margin: 0 }}
