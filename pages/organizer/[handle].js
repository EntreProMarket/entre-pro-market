...
export default function OrganizerPublicProfile() {
  ...
  const handleBack = () => {
    ...
    if (fromParam === "admin") { router.push("/admin?tab=Users"); return; }
  ...
  return (
    <div style={{ maxWidth: 800, margin: "auto", padding: 20, position: "relative", fontFamily: "sans-serif" }}>
      ...
      {organizer.logo_url && (
        <div onClick={() => setSelectedImage(organizer.logo_url)} style={logoBoxStyle}>
          <img src={organizer.logo_url} alt="logo" style={{ width: "100%", height: "auto", display: "block" }} />
        </div>
      )}
