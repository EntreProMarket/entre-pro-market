...
function getMessagingRules(role, tier) {
  if (role === "vendor") {
    if (tier === "premium") return { canMessage: true, canMessageVendors: true, canMessageOrganizers: false, canInitiateOrganizer: false, canSaveContacts: true, label: "Premium Vendor" };
    if (tier === "featured") return { canMessage: true, canMessageVendors: true, canMessageOrganizers: true, canInitiateOrganizer: true, canSaveContacts: true, label: "Featured Vendor" };
    return { canMessage: false, label: "Free Vendor" };
  }
  if (role === "organizer") {
    if (tier === "basic") return { canMessage: true, canMessageVendors: true, vendorLimit: 5, canMessageOrganizers: true, canSaveContacts: false, label: "Basic Organizer" };
    if (tier === "pro") return { canMessage: true, canMessageVendors: true, vendorLimit: 20, canMessageOrganizers: true, canSaveContacts: true, label: "Pro Organizer" };
    if (tier === "elite") return { canMessage: true, canMessageVendors: true, vendorLimit: null, canMessageOrganizers: true, canSaveContacts: true, label: "Elite Organizer" };
    if (tier === "premium") return { canMessage: true, canMessageVendors: true, vendorLimit: 20, canMessageOrganizers: true, canSaveContacts: false, label: "Pro Organizer" };
...
export default function Messages() {
  ...
  const canMessagePartner = (partner) => {
    ...
    if (profile?.role === "vendor") {
      if (partner.role === "vendor") return rules.canMessageVendors;
      if (partner.role === "organizer") { if (rules.canInitiateOrganizer) return true; return messages.some(m => m.sender_id === partner.id); }
    }
    if (profile?.role === "organizer") {
      if (partner.role === "organizer") return rules.canMessageOrganizers;
      if (partner.role === "vendor") { if (!rules.vendorLimit) return true; return vendorContactCount < rules.vendorLimit; }
    }
    return false;
  };
  ...
  const sendMessage = async () => {
    ...
    if (!error) { setNewMessage(""); await loadMessages(activeConvo); await loadConversations(userId); if (profile?.role === "organizer") setVendorContactCount(c => c + 1); }
    setSending(false);
  };
      ...
      <DashboardLayout>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "70vh", textAlign: "center", padding: 30 }}>
          ...
          <p style={{ color: "#666", fontSize: 15, maxWidth: 320, lineHeight: 1.6, marginBottom: 32 }}>
            {profile?.role === "organizer" ? "Your account needs an active subscription to access messaging." : "Messaging is available to Premium & Featured Vendors and all Organizer plans."}
          </p>
          ...
          <button onClick={() => router.push("/organizer-info")} style={{ padding: "14px 32px", backgroundColor: "#AABB23", color: "white", border: "none", borderRadius: 30, fontWeight: "bold", fontSize: 16, cursor: "pointer", marginBottom: 24, width: "100%", maxWidth: 280 }}>View Organizer Plans</button>
          <p onClick={() => router.back()} style={{ color: "#701890", cursor: "pointer", fontSize: 14, textDecoration: "underline" }}>← Go Back</p>
        </div>
      </DashboardLayout>
    );
  }
    ...
    <DashboardLayout>
      <div style={{ maxWidth: 900, fontFamily: "sans-serif" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16, minHeight: 500 }}>
          ...
          {/* CONVERSATIONS LIST */}
          <div style={{ backgroundColor: "white", border: "1px solid #eee", borderRadius: 10, overflow: "hidden", display: activeConvo ? "none" : "block" }}>
                ...
                return (
                  <div key={convo.partnerId} style={{ borderBottom: "1px solid #f5f5f5", backgroundColor: hasUnread ? "#faf5ff" : "white", display: "flex", alignItems: "center", position: "relative" }}>
                    <div onClick={() => { setActiveConvo(convo.partnerId); loadPartner(convo.partnerId); }}
                      style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, cursor: "pointer", padding: "14px 36px 14px 16px" }}>
                      {/* Avatar with unread dot */}
                      <div style={{ position: "relative", flexShrink: 0 }}>
                        {convo.partner?.logo_url ? (
                          <img src={convo.partner.logo_url} style={{ width: 42, height: 42, borderRadius: "50%", objectFit: "cover", border: hasUnread ? "2px solid #701890" : "2px solid transparent" }} />
                        ) : (
                          <div style={{ width: 42, height: 42, borderRadius: "50%", backgroundColor: "#eee", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, border: hasUnread ? "2px solid #701890" : "2px solid transparent" }}>👤</div>
                        )}
                        {hasUnread && (
                          <span style={{ position: "absolute", top: -2, right: -2, backgroundColor: "#AABB23", color: "white", borderRadius: "50%", width: 18, height: 18, fontSize: 10, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: "bold", border: "2px solid white" }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        {/* Bold name if unread */}
                        <p style={{ margin: 0, fontWeight: hasUnread ? "900" : "bold", fontSize: 14, color: hasUnread ? "#701890" : "#222", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                          {getName(convo.partner)}
                        </p>
                        {/* Bold preview if unread */}
                        <p style={{ margin: 0, fontSize: 12, color: hasUnread ? "#333" : "#888", fontWeight: hasUnread ? "bold" : "normal", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                          {convo.lastMessage?.content}
                        </p>
                      </div>
                      {/* Unread indicator dot on right */}
                      {hasUnread && (
                        <div style={{ width: 10, height: 10, borderRadius: "50%", backgroundColor: "#701890", flexShrink: 0 }} />
                      )}
                    </div>
                    <button
                      onClick={async (e) => {
                      ...
                      style={{ position: "absolute", top: 8, right: 8, background: "none", border: "none", color: "#bbb", cursor: "pointer", fontSize: 16, padding: 4, lineHeight: 1 }}>✕</button>
                  </div>
                );
              })
            )}
            ...
            <div style={{ backgroundColor: "white", border: "1px solid #eee", borderRadius: 10, display: "flex", flexDirection: "column", overflow: "hidden" }}>
              <div style={{ padding: "12px 16px", borderBottom: "1px solid #eee" }}>
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {activePartner?.handle && (
                    <button onClick={() => router.push(`/${activePartner.role}/${activePartner.handle}`)} style={{ padding: "8px 20px", backgroundColor: "#701890", color: "white", border: "none", borderRadius: 20, cursor: "pointer", fontSize: 13, fontWeight: "bold" }}>View Profile</button>
                  )}
                  ...
                  messages.map(msg => {
                    ...
                    return (
                      <div key={msg.id} style={{ display: "flex", justifyContent: isMine ? "flex-end" : "flex-start", alignItems: "flex-end", gap: 6 }}>
                        <div style={{ maxWidth: "75%", padding: "10px 14px", borderRadius: isMine ? "16px 16px 4px 16px" : "16px 16px 16px 4px", backgroundColor: isMine ? "#701890" : "#f0f0f0", color: isMine ? "white" : "#333", fontSize: 13, lineHeight: 1.5 }}>
                          {msg.content}
              ...
              {canMessagePartner(activePartner) ? (
                <div style={{ padding: "12px 16px", borderTop: "1px solid #eee", display: "flex", gap: 10 }}>
                  <input value={newMessage} onChange={e => setNewMessage(e.target.value)} onKeyDown={e => e.key === "Enter" && !e.shiftKey && sendMessage()} placeholder="Type a message... (Enter to send)" style={{ flex: 1, padding: "10px 14px", borderRadius: 20, border: "1px solid #ddd", fontSize: 13, outline: "none" }} />
                  <button onClick={sendMessage} disabled={sending || !newMessage.trim()} style={{ padding: "10px 18px", backgroundColor: "#701890", color: "white", border: "none", borderRadius: 20, cursor: "pointer", fontWeight: "bold", fontSize: 13, opacity: !newMessage.trim() ? 0.5 : 1 }}>
                    {sending ? "..." : "Send"}
                  </button>
                </div>
              ) : (
                <div style={{ padding: "12px 16px", borderTop: "1px solid #eee", textAlign: "center" }}>
                  <p style={{ color: "#888", fontSize: 13, margin: 0 }}>
                    {rules.vendorLimit && vendorContactCount >= rules.vendorLimit ? `Monthly limit reached (${rules.vendorLimit}). Upgrade to send more.` : "You cannot message this user on your current plan."}
                  </p>
        ...
        {reportModal && (
          <div onClick={() => setReportModal(false)} style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", backgroundColor: "rgba(0,0,0,0.5)", display: "flex", justifyContent: "center", alignItems: "center", zIndex: 9999 }}>
            <div onClick={e => e.stopPropagation()} style={{ backgroundColor: "white", borderRadius: 12, padding: 24, maxWidth: 400, width: "90%" }}>
              ...
              <p style={{ color: "#666", fontSize: 13 }}>Tell us why you're reporting this message.</p>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
                ...
                <button onClick={reportMessage} disabled={!reportReason} style={{ padding: "10px 18px", backgroundColor: "#cc0000", color: "white", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: "bold", opacity: !reportReason ? 0.5 : 1 }}>Submit Report</button>
              </div>
            </div>
          </div>
        )}
