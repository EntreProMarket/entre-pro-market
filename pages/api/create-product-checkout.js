...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: product } = await supabaseAdmin.from("vendor_products").select("*").eq("id", productId).single();
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          currency: "usd",
          product_data: { name: product.title, description: product.description || "", images: product.image_url ? [product.image_url] : [] },
      ...
      metadata: { productId: product.id, buyerId: userId, vendorId: product.vendor_id },
    });
