...
export default function VendorProfile() {
  ...
  return (
    <div style={{ maxWidth: 700, margin: "auto", padding: 20, fontFamily: "sans-serif" }}>
      ...
      {/* PROFILE TAB - Everything you had before */}
      {activeTab === "profile" && (
        <>
          ...
          <div style={{ marginTop: 20, marginBottom: 8 }}>
            ...
            <div style={{ backgroundColor: "#fff8e1", border: "1px solid #f0c040", borderRadius: 6, padding: "8px 12px", marginBottom: 10, fontSize: 12, color: "#856404" }}>
              ⚠️ Accepted: JPG, PNG, WebP only. HEIC not supported.
            </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(100px, 1fr))", gap: 8, marginBottom: 12 }}>
                {portfolioImages.map((img, i) => (
                  <div key={i} style={{ position: "relative" }}>
                    ...
                    <button onClick={() => setPortfolioImages(portfolioImages.filter(x => x !== img))} style={{ position: "absolute", top: 2, right: 2, background: "rgba(0,0,0,0.6)", color: "white", border: "none", borderRadius: "50%", width: 20, height: 20, fontSize: 11, cursor: "pointer" }}>×</button>
                  </div>
                ))}
              </div>
            )}
      ...
      {activeTab === "shop" && (
        <div>
          ...
          <button onClick={addShopProduct} style={{ padding: "14px 28px", backgroundColor: "#701890", color: "white", border: "none", borderRadius: 8, fontWeight: "bold" }}>
            Add to Shop
          </button>
            ...
            {shopProducts.map(p => (
              <div key={p.id} style={{ border: "1px solid #ddd", borderRadius: 8, overflow: "hidden" }}>
                ...
                <div style={{ padding: 10 }}>
                  ...
                  <p style={{ margin: 0, color: "#701890", fontSize: 16, fontWeight: "bold" }}>${(p.price / 100).toFixed(2)}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      {message && (
        <p style={{ padding: "12px 16px", backgroundColor: message.startsWith("✅") ? "#f0fdf4" : "#fef2f2", border: `1px solid ${message.startsWith("✅") ? "#86efac" : "#fca5a5"}`, borderRadius: 6, marginTop: 16 }}>
          {message}
        </p>
      )}
