...
export default function AdminDashboard() {
  ...
  const checkAdmin = async () => {
    ...
    if (!profile) { console.log("Profile load failed - checking session"); return; }
  ...
  const loadAllData = async () => {
    const { count: totalVendors } = await supabase
      .from("profiles").select("*", { count: "exact", head: true }).eq("role", "vendor");
    const { count: totalOrganizers } = await supabase
      .from("profiles").select("*", { count: "exact", head: true }).eq("role", "organizer");
    const { count: premiumVendors } = await supabase
      .from("profiles").select("*", { count: "exact", head: true }).eq("role", "vendor").eq("account_type", "premium");
    const { count: featuredVendors } = await supabase
      .from("profiles").select("*", { count: "exact", head: true }).eq("role", "vendor").eq("account_type", "featured");

    setStats({ totalVendors: totalVendors || 0, totalOrganizers: totalOrganizers || 0, premiumVendors: premiumVendors || 0, featuredVendors: featuredVendors || 0 });
  ...
  const saveLimits = async () => {
    ...
    setMessage(hasError ? "❌ Some limits failed to save. Check console." : "✅ Limits saved! Changes are live immediately.");
    setSaving(false);
  };
  ...
  const savePlan = async (plan) => {
    ...
    setMessage(error ? "❌ Error saving plan: " + error.message : "✅ Plan saved!");
    setSaving(false);
  };
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#f8f9fa", fontFamily: "sans-serif" }}>
      ...
      <div style={{ backgroundColor: "#111", color: "white", padding: "14px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button onClick={() => window.location.replace('/home')} style={{ background: "none", border: "1px solid #555", color: "white", padding: "6px 12px", borderRadius: 20, cursor: "pointer", fontSize: 12, whiteSpace: "nowrap" }}>← Home</button>
          <span style={{ fontWeight: "bold", fontSize: 16, whiteSpace: "nowrap" }}>Entre PRO Market</span>
          <span style={{ backgroundColor: "#701890", color: "white", fontSize: 10, padding: "3px 8px", borderRadius: 10, fontWeight: "bold", whiteSpace: "nowrap" }}>ADMIN</span>
        </div>
        <button onClick={logout} style={{ padding: "6px 14px", backgroundColor: "#ff6b6b", color: "white", border: "1px solid #ff6b6b", borderRadius: 20, cursor: "pointer", fontWeight: "bold", fontSize: 12, whiteSpace: "nowrap" }}>Log Out</button>
      </div>

      {/* TABS */}
      <div style={{ backgroundColor: "white", borderBottom: "1px solid #eee", display: "flex", overflowX: "auto", padding: "0 24px" }}>
        {TABS.map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: "14px 18px", border: "none", borderBottom: activeTab === tab ? "3px solid #701890" : "3px solid transparent", backgroundColor: "transparent", color: activeTab === tab ? "#701890" : "#666", fontWeight: activeTab === tab ? "bold" : "normal", cursor: "pointer", fontSize: 14, whiteSpace: "nowrap" }}>
            {tab}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: 24, maxWidth: 1000, margin: "0 auto" }}>
        ...
        {activeTab === "Plans & Pricing" && (
          <div>
            ...
            <p style={{ color: "#888", marginBottom: 24, fontSize: 14 }}>Edit prices and features here — changes go live immediately.</p>
            {plans.length === 0 ? (
                  ...
                  <div key={plan.id} style={{ backgroundColor: "white", border: "1px solid #eee", borderRadius: 10, padding: 20 }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
                      ...
                      <div>
                        ...
                        <input type="number" value={plan.price || ""} onChange={e => setPlans(plans.map((p, idx) => idx === i ? { ...p, price: e.target.value } : p))} style={inputStyle} placeholder="0 = Free" />
                      </div>
                    </div>
        ...
        {activeTab === "Users" && (
          <div>
            ...
            <p style={{ color: "#888", marginBottom: 24, fontSize: 14 }}>Manage user tiers, suspend accounts, and view all profiles.</p>
        ...
        {activeTab === "Featured Vendors" && (
          <div>
            ...
            <p style={{ color: "#888", marginBottom: 24, fontSize: 14 }}>Featured vendors appear at the top of the marketplace with a 🔥 badge.</p>
        ...
        {activeTab === "Premium Vendors" && (
          <div>
            ...
            <p style={{ color: "#888", marginBottom: 24, fontSize: 14 }}>Manage premium upgrades and downgrades here.</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {users.filter(u => u.role === "vendor").map((user) => (
        ...
        {activeTab === "Ads" && (
          <div>
            ...
            <p style={{ color: "#888", marginBottom: 24, fontSize: 14 }}>Edit ad banners that appear across the marketplace.</p>
            {ads.length === 0 ? (
              <div style={{ backgroundColor: "white", border: "1px solid #eee", borderRadius: 10, padding: 30, textAlign: "center", color: "#888" }}>
        ...
        {activeTab === "Reports" && (
          <div>
            ...
            {reports.length === 0 ? (
              <div style={{ backgroundColor: "white", border: "1px solid #eee", borderRadius: 10, padding: 30, textAlign: "center", color: "#888" }}><p>No reports yet. 🎉</p></div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {reports.map((report) => (
                  <div key={report.id} style={{ backgroundColor: "white", border: `1px solid ${report.status === "pending" ? "#fca5a5" : "#eee"}`, borderRadius: 10, padding: 16 }}>
                    ...
                    <div style={{ backgroundColor: "#f9f9f9", borderRadius: 6, padding: "10px 14px", marginBottom: 12 }}>
                      ...
                      {report.message?.content && <p style={{ margin: "8px 0 0", fontSize: 12, color: "#444", fontStyle: "italic" }}>Message: "{report.message.content}"</p>}
                    </div>
        ...
        {activeTab === "Settings" && (
          <div>
            ...
            <p style={{ color: "#888", marginBottom: 24, fontSize: 14 }}>Global app settings and configuration.</p>
            ...
            <div style={{ backgroundColor: "white", border: "1px solid #eee", borderRadius: 10, padding: 24, marginBottom: 20 }}>
              ...
              <p style={{ color: "#888", fontSize: 13, marginBottom: 20 }}>
                Changes save instantly and apply to all uploads immediately — no code change needed.
              </p>
            ...
            <div style={{ backgroundColor: "#fff8e1", border: "1px solid #f0c040", borderRadius: 10, padding: 20 }}>
              ...
              <p style={{ fontSize: 13, color: "#856404" }}>These actions are irreversible. Use with caution.</p>
