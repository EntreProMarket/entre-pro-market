...
// Available to Featured Vendors and Elite Organizers only.
...
export default function ProfileInsights() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const isEligible =
        (p.role === "vendor" && p.account_type === "featured") ||
        (p.role === "organizer" && p.account_type === "elite");

      if (!isEligible) {
        router.replace(p.role === "vendor" ? "/vendor-dashboard" : "/organizer-dashboard");
        return;
      }
  ...
  const getName = (p) => p?.business_name || p?.organizer_name || "Anonymous";
      ...
      <div style={{ maxWidth: 700, fontFamily: "sans-serif" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 24 }}>
          <div style={{ backgroundColor: "#f3e8ff", border: "1px solid #701890", borderRadius: 10, padding: "16px 20px", textAlign: "center" }}>
            <p style={{ fontSize: 32, fontWeight: "bold", color: "#701890", margin: 0 }}>{views.length}</p>
            <p style={{ fontSize: 13, color: "#701890", margin: "4px 0 0", fontWeight: "bold" }}>Profile Views</p>
          </div>
        ...
        <div style={{ display: "flex", borderBottom: "2px solid #eee", marginBottom: 20 }}>
          <button onClick={() => handleTabChange("views")} style={{ flex: 1, padding: 12, border: "none", borderBottom: activeTab === "views" ? "3px solid #701890" : "3px solid transparent", backgroundColor: "transparent", color: activeTab === "views" ? "#701890" : "#666", fontWeight: activeTab === "views" ? "bold" : "normal", cursor: "pointer", fontSize: 14 }}>
            👁️ Profile Views
          </button>
          <button onClick={() => handleTabChange("messages")} style={{ flex: 1, padding: 12, border: "none", borderBottom: activeTab === "messages" ? "3px solid #701890" : "3px solid transparent", backgroundColor: "transparent", color: activeTab === "messages" ? "#701890" : "#666", fontWeight: activeTab === "messages" ? "bold" : "normal", cursor: "pointer", fontSize: 14 }}>
            ✉️ Messages
          </button>
        </div>
        ...
        {activeTab === "views" && (
          <div>
            {loadingData ? <p style={{ color: "#888", textAlign: "center" }}>Loading views...</p> :
              views.length === 0 ? (
                <div style={{ backgroundColor: "white", border: "1px solid #eee", borderRadius: 10, padding: 32, textAlign: "center", color: "#aaa" }}>
                  ...
                  <p style={{ fontSize: 14, marginTop: 12 }}>No profile views yet. Share your profile to get discovered!</p>
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  {views.map((v, i) => (
                    <div key={v.id || i} style={{ backgroundColor: "white", border: "1px solid #eee", borderRadius: 10, padding: "12px 16px", display: "flex", alignItems: "center", gap: 12 }}>
                      <div style={{ width: 40, height: 40, borderRadius: 8, overflow: "hidden", border: "1px solid #e5e7eb", flexShrink: 0 }}>
                        {v.viewer?.logo_url ? <img src={v.viewer.logo_url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} /> : <div style={{ width: "100%", height: "100%", backgroundColor: "#f5f5f5", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>👤</div>}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                          <span style={{ fontWeight: "bold", fontSize: 14 }}>
                            {v.viewer ? getName(v.viewer) : "Anonymous Visitor"}
                          </span>
                          {v.viewer?.account_type && tierBadge(v.viewer.account_type)}
                          {v.viewer?.role && <span style={{ fontSize: 10, color: "#888", textTransform: "capitalize" }}>{v.viewer.role}</span>}
                        </div>
                        {v.viewer?.handle && <p style={{ margin: 0, fontSize: 12, color: "#aaa" }}>@{v.viewer.handle}</p>}
                        <p style={{ margin: 0, fontSize: 11, color: "#bbb", marginTop: 2 }}>{formatDate(v.created_at)}</p>
                      </div>
                      {v.viewer?.handle && (
                        <button onClick={() => router.push(`/${v.viewer.role}/${v.viewer.handle}`)}
        ...
        {activeTab === "messages" && (
          <div>
            {loadingData ? <p style={{ color: "#888", textAlign: "center" }}>Loading messages...</p> :
              messages.length === 0 ? (
                <div style={{ backgroundColor: "white", border: "1px solid #eee", borderRadius: 10, padding: 32, textAlign: "center", color: "#aaa" }}>
                  <p style={{ fontSize: 36, margin: 0 }}>✉️</p>
                  ...
                  {messages.map((msg, i) => (
                    <div key={msg.id || i} onClick={() => router.push(`/messages?to=${msg.sender_id}`)}
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <p style={{ margin: 0, fontSize: 11, color: "#bbb", marginTop: 2 }}>{formatDate(msg.created_at)}</p>
                      </div>
                    </div>
                  ))}
                </div>
